...
class Endpoint:
    ...
    def match(self, path: str) -> Union[bool, list]:
    ...
    def add_endpoint(self, path: str, methods: List[str] = ["GET"]) -> Callable:
        ...
        def wrapper(handler: Coroutine) -> Coroutine:
            ...
            if all(char in path for char in ("<", ">")) and not isinstance(path, re.Pattern):
                ...
                self.endpoints.add(Endpoint(new_path, methods, handler))
                return handler
                
            self.endpoints.add(Endpoint(path, methods, handler))
            return handler
        return wrapper
...
class LenHTTP:
    ...
    def start(self):
        ...
        async def runner():
            ...
            while True:
                ...
                for rd in rlist:
                    if rd is sock:
                        ...
                        raise RuntimeError(f'Unknown reader {rd}')
            ...
            if self.tasks:
                warning("Canceling all active tasks..")
